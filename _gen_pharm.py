...
DATA_DIR = os.path.join(ROOT, "pharm_data")
OUT_DIR = ROOT
...
RENAL_IDENTITY = "#f43f5e"  # amber for index/header
RENAL_IDENTITY_RGB = (244, 63, 94)
...
def make_html(game, prev_file, next_file, idx_in_palette):
    ...
    # Nav links
    index_link = (
        '<a href="pharm_index.html" style="position:fixed;top:12px;left:12px;z-index:9999;'
...
def main():
    ...
    for i, g in enumerate(all_games):
        g["file"] = f"pharm_{i+1:02d}_{g['slug']}.html"
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>PharmStudy &ndash; Drag &amp; Drop Games</title>
<style>
  ...
  *{{box-sizing:border-box;margin:0;padding:0;}}
  ...
  header::after{{content:'\\1F48A';position:absolute;font-size:360px;opacity:.025;top:50%;left:50%;transform:translate(-50%,-50%);pointer-events:none;}}
...
<header>
  <h1>PharmStudy</h1>
  <p>Drag &amp; Drop Study Games &nbsp;&middot;&nbsp; Pharmacology</p>
  <div class="pills">
...
document.getElementById('search').addEventListener('input', function() {{
  ...
  document.getElementById('noResults').style.display = anyVisible ? 'none' : 'block';
    ...
    with open(os.path.join(OUT_DIR, "pharm_index.html"), "w", encoding="utf-8") as f:
        f.write(index_html)
